...
<br>
    <section class="tablaEquipos" style="max-width: 600px;">
         <section class="tituloRegistroMaestro" style="width: 600px;">
            <p>TABLA DE UNIDADES ACADÉMICAS REGISTRADAS</p><br>
        </section>
      ...
      ?>
                <tr>
                  ...
                  <td><a href="editarUnidad.php?id=<?php echo $row['idUnidadAcademica']; ?>"><?=$row['nombreUnidadAcademica'];?></a></td>
                  <td><button id="eliminar" name="<?=$row['nombreUnidadAcademica'];?>" value="<?=$row['idUnidadAcademica'];?>">Eliminar</button></td>
                </tr>
      <?php
      }
      ?>  
    ...
    $(document).ready(function(){
      $("section div table tbody tr td button#eliminar").click(function(){
        ...
        if(confirmar==true){
          $.ajax({ 
              type: "POST",
              url: "views/actionEliminarUnidad.php",
              data:"id="+id
            }).done(function(){
